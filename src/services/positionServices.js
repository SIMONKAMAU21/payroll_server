...
export const getAllpositionByIdServices = async (PositionID) => {
  try {
    const result = await poolRequest()
      .input('PositionID', sql.Int, PositionID)
      .query(`SELECT Positions.*, Employees.*
              FROM Positions
              INNER JOIN Employees ON Positions.PositionID = Employees.PositionID
...
export const addPositionServices = async (newPosition) => {
  try {
    const result = await poolRequest()
      ...
      .query(
        `INSERT INTO Positions (Position,Basic_Salary) VALUES(@Position,@Basic_Salary)`
      );
    return result;
  } catch (error) {
...
export const updatepositionServices = async (
  PositionID,
  updatedpositionData
) => {
  try {
